...
class LocalNorm2d(nn.Module):
    ...
    def forward(self,x):
        ...
        return torch.clamp((x - mean) / (torch.sqrt(torch.abs(self.pool(F.pad(x*x,  (pd,pd,pd,pd), 'reflect')) - mean*mean )) + 1e-7), min = -6.0, max = 6.0)
...
class AffNetFastFullAff(nn.Module):
    def __init__(self, PS = 32):
        super(AffNetFastFullAff, self).__init__()
    ...
    def weights_init(self,m):
        ...
        return
    def forward(self, input, return_A_matrix = False):
        xy = self.features(self.lrn(input))#.view(-1,3)
...
class AffOriNetFast(nn.Module):
    def __init__(self, PS = 32):
        super(AffOriNetFast, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding=1, bias = False),
            nn.BatchNorm2d(16, affine=False),
            nn.ReLU(),
